...
public class HealthBarVisual : MonoBehaviour
{
    ...
    void Start()
    {
        if (firewallHealth == null)
        {
            ...
            if (firewallHealth == null)
            {
                Debug.LogError("FirewallHealth script not found in scene!");
                return;
            }
        }
        ...
        // setup the gradient (Green → Yellow → Orange → Red)
        SetupGradient();
    ...
    void SetupGradient()
    {
        ...
        // at 33% health 
        colorKeys[2] = new GradientColorKey(new Color(1f, 0.5f, 0f), 0.33f); // Orange
    ...
    void UpdateHealthBar()
    {
        ...
        if (isLowHealth && enablePulseEffect)
        {
            ...
            float alphaMultiplier = 0.7f + (pulseValue * pulseAlphaIntensity);
